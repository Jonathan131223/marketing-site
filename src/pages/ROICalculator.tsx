                  ...
                  <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">
                        % of free/trial signups that currently become paying customers.
                      </p>
                      {errors.currentConversionRate && (
                        <p className="text-sm text-destructive">{errors.currentConversionRate}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-sm text-muted-foreground">
                        Average number of new free or trial users who sign up each month.
                      </p>
                      {errors.freeUsersPerMonth && (
                        <p className="text-sm text-destructive">{errors.freeUsersPerMonth}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-sm text-muted-foreground">
                        Total revenue you earn from a paying customer over their lifetime.
                      </p>
                      {errors.clv && (
                        <p className="text-sm text-destructive">{errors.clv}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {errors.estimatedLift && (
                        <p className="text-sm text-destructive">{errors.estimatedLift}</p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) => set("serviceCost", value)}
                      >
                        <SelectTrigger id="serviceCost" className="w-full">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Which DigiStorms plan are you considering?{" "}
                        <a href="/pricing" className="underline hover:text-foreground transition-colors">
                          See all plans →
                        </a>
                      </p>
                      {errors.serviceCost && (
                        <p className="text-sm text-destructive">{errors.serviceCost}</p>
                      )}
                    </div>
                    ...
                    {!results ? (
                      <div className="flex items-center justify-center h-full min-h-[300px] text-center">
                        <p className="text-muted-foreground">
                          Enter your numbers and click "Calculate ROI" to see your potential results.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-6">
                        ...
                        <div className="p-6 bg-primary/10 rounded-lg">
                          ...
                          {results.roi === null ? (
                            ...
                            <p className="text-4xl font-bold text-destructive">
                              –{formatNumber(Math.abs(parseFloat(results.roi)))}%
                            </p>
                          ) : (
                        ...
                        {roiIsNegative ? (
                          <p className="text-sm text-destructive">
                            No gain with these inputs. Try adjusting your numbers or selecting a lower-cost plan.
                          </p>
                        ) : (
                          <>
                            <div className="space-y-4">
                              <div className="flex justify-between items-center pb-2 border-b border-border">
                                ...
                                <span className="font-semibold text-foreground text-sm">
                                  {results.currentConversionRate}% → {results.newConversionRate}%
                                </span>
                              </div>
                              <div className="flex justify-between items-center pb-2 border-b border-border">
                                <span className="text-muted-foreground text-sm">Paid users / month</span>
                                <span className="font-semibold text-foreground text-sm">
                                  {formatNumber(results.baselinePaidUsers)} → {formatNumber(results.newPaidUsers)}
                                </span>
                              </div>
                              <div className="flex justify-between items-center pb-2 border-b border-border">
                                <span className="text-muted-foreground text-sm">Baseline revenue</span>
                                <span className="font-semibold text-foreground text-sm">
                                  ${formatNumber(results.baselineRevenue)}
                                </span>
                              </div>
                              <div className="flex justify-between items-center pb-2 border-b border-border">
                                <span className="text-muted-foreground text-sm">New revenue</span>
                                <span className="font-semibold text-foreground text-sm">
                                  ${formatNumber(results.newRevenue)}
                                </span>
                              </div>
                              <div className="flex justify-between items-center pb-2 border-b border-border">
                                <span className="text-muted-foreground text-sm">Additional revenue / month</span>
                              ...
                              {results.serviceCost > 0 && (
                                <div className="flex justify-between items-center pb-2 border-b border-border">
                                  <span className="text-muted-foreground text-sm">Plan cost</span>
                                  <span className="font-semibold text-foreground text-sm">
                                    ${formatNumber(results.serviceCost)}/mo
                                  </span>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="p-4 bg-muted rounded-lg text-sm text-muted-foreground">
                              Improving conversion from {results.currentConversionRate}% to{" "}
                              {results.newConversionRate}% could add{" "}
                              <strong className="text-foreground">
                                +${formatNumber(results.revenueGain)}
                              </strong>{" "}
                              in revenue each month from new conversions
                              {results.roi !== null && results.serviceCost > 0
                                ? ` — a ${formatNumber(parseFloat(results.roi))}% ROI on a $${formatNumber(results.serviceCost)}/mo plan.`
                                : "."}
                            </div>
                          </>
                        )}
                ...
                {results && !roiIsNegative && (
                  <div className="mt-10 p-6 bg-primary/5 rounded-lg border border-primary/20 text-center">
                    ...
                    <p className="text-muted-foreground mb-4">
                      DigiStorms builds your onboarding email sequence automatically — so you can start moving users from signup to paid.
                    </p>
